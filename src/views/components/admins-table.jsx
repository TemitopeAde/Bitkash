...
export default function AdminsTable({ tableInstance, classname, children }) {
  const {
    getTableProps,
    getTableBodyProps,
    canNextPage,
          ...
          {headerGroups.map((headerGroup) => (
            <tr {...headerGroup.getHeaderGroupProps()}>
              {headerGroup.headers.map((column) => (
                <th className='tw-text-start tw-px-6 tw-py-3 tw-text-base tw-font-poppins tw-text-primary-main'>
          ...
          {page.map((row) => {
            ...
            return (
              <tr {...row.getRowProps()} className='tw-bg-white'>
                {row.cells.map((cell) => (
                  <td className='tw-border-b-2 tw-max-w-[100px] tw-truncate tw-border-primary-main tw-px-6 tw-py-4 tw-font-poppins'>
                    {cell.render("Cell")}
                  </td>
                ))}
              </tr>
